...
const { SKIP_ADS, SKIP_EMPTY_UIDS } = config;

type ListData = Pick<Article, 'time' | 'href' | 'img_src' | 'tags' | 'content' | 'cat'>;

type DetailData = Pick<Article, 'title' | 'entry_content' | 'rating_count' | 'rating_score' | 'uid' | 'imgs' | 'raw_id'>;

type CrawArticle = Omit<Article, 'id'>;

export class ArticleCraw extends BaseCraw {
  constructor(private startPage: number = 1, private endPage?: number) {
    super();
  }
  ...
  parseDetail = async (uri: string, listData: ListData) => {
    ...
    const rating_count = $('.post-ratings strong').eq(0).text().trim().replace(',', '');
  ...
  parseList = async (link: string) => {
    ...
    $('article.post').each((_, el) => {
      ...
      if ((SKIP_ADS && !title) || adArr.some(adStr => testStr.includes(adStr))) {
        return;
      }
      if (!href) {
  ...
  run = async () => {
    ...
    const links = range(this.startPage, this.endPage + 1).map(i => `${config.BASE_LINK}/${i}`);
    const queue = new PQueue({
      concurrency: 5,
      interval: 1000,
      intervalCap: 3,
    });
    await queue.addAll(links.map(link => () => this.withErrorHandler(this.parse)(link)));
  };
}
