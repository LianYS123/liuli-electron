...
export const Setting: React.FC = () => {
  const [config, _setConfig] = useState<CrawConfig>({ BASE_LINK: '', PROXY: '', SKIP_ADS: false, SKIP_EMPTY_UIDS: false });
  ...
  const { data: { errors, insertCount, pending, startTime, updateCount, endTime } = {}, isIdle, isStale, isError } = useQuery(
    'ArticleCrawPending',
    () => articleCrawAPI.stat(),
    {
  ...
  const { data: totalPages } = useQuery(['ArticleCrawGetEndPage'], () => articleCrawAPI.getEndPage(), {})


  const { run: handleCraw } = useDebounceFn(craw, { wait: 100 })


  const { run: handleSave } = useDebounceFn(async () => {
      ...
      <Stack spacing={2}>
        ...
        <Box sx={{ mt: 2 }}>
          <Paper sx={{ padding: '8px', color: t => t.palette.text.secondary }}>
            <Stack spacing={1}>
              <Box>
                开始时间：{formatTimeDetail(startTime) || "--"}
              </Box>
              <Box>
                结束时间：{formatTimeDetail(endTime) || "--"}
              </Box>
              <Box>
                耗时：{getCost() || "--"}
              </Box>
